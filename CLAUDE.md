...
- **Graph node/edge IDs are deterministic**: entity nodes use `entity:{md5(type:label)[:16]}` via `builder._stable_hash()` — never random UUIDs (random IDs duplicated every entity node on re-ingest because `ON CONFLICT` never fired). Co-occurrence edges are canonical (`sorted(pair)` → one direction only); similarity edge IDs embed FULL embed_ids (no `[-8:]` truncation — it silently dropped edges on suffix collisions). Node `properties` are built with `json.dumps`, never f-strings.
- **graph_path truncation**: `orchestrator._truncate_graph_path()` keeps seed (vector-hit) chunks first, fills with highest-degree nodes, and drops edges whose endpoints were cut. Never use a blind `nodes[:40]` slice — it can drop the seed chunks and leaves dangling edges React Flow silently discards.
- **Scorer conflict detection**: `scorer.rank_evidence()` attributes incidents to entities via `mentions` edges (chunk→entity); an entity mentioned by chunks from 2+ incidents gets `conflict: true`. Requires graph_edges to be passed — label-based detection was dead code (entities never carried incident ids).
- **`/graph/preloaded/{domain}` node budget**: chunks capped at ~60% of `max_nodes` so entities can never be starved to a 0-row LIMIT (the old `max_nodes - len(chunk_ids)` computed to 0 and returned a disconnected chunk-only graph); entities are selected by degree DESC.
- **Shell scripts must be LF**: `.gitattributes` forces `eol=lf` for `*.sh` — a CRLF `entrypoint.sh` in the Docker image fails with `env: 'bash\r': No such file or directory` (exit 127) when built from a Windows checkout.
- **Compose demo mounts**: `docker-compose.yml` mounts `./demo` at BOTH `/workspace/backend/demo` (LightRAG demo indexer) and `/workspace/demo` (`kaggle_loader.py` resolves seed CSVs from repo root) — removing either breaks its consumer.
